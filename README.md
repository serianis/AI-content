...
1. **Upload Plugin Files**
   - Download or clone this repository into `/wp-content/plugins/autoblogai/`
...
3. **Configure API Key**
   - Go to AutoblogAI menu in admin
...
Access these settings via **Admin Dashboard > AutoblogAI**:
...
```php
// API Configuration
get_option( 'autoblogai_api_key' );          // Encrypted
get_option( 'autoblogai_language' );         // 'Greek', 'English', etc.
get_option( 'autoblogai_tone' );             // 'Professional', 'Casual', 'Journalistic'
get_option( 'autoblogai_post_status' );      // 'draft', 'publish'

// Scheduling & Queue
get_option( 'autoblogai_daily_publish_cap' );   // Integer
get_option( 'autoblogai_publish_queue' );       // Array of queued topics
get_option( 'autoblogai_last_publish_date' );   // Last publish timestamp
```
...
**Event**: `autoblogai_daily_publish_event`
...
update_option( 'autoblogai_api_key', $encrypted );

// Decrypt when using
$plain_key = $security->decrypt_api_key( get_option( 'autoblogai_api_key' ) );
```
...
### Database Table: `wp_autoblogai_logs`
...
Access logs via **Admin Dashboard > AutoblogAI > Logs**
...
$next_run = wp_next_scheduled( 'autoblogai_daily_publish_event' );
```
