...
class AutoblogAI_Post_Generator {
    public const OPTION_BANNED_WORDS           = 'autoblogai_banned_words';
    public const OPTION_SIMILARITY_THRESHOLD   = 'autoblogai_similarity_threshold';
    public const OPTION_MAX_POSTS_PER_DAY      = 'autoblogai_max_posts_per_day';
    public const TRANSIENT_DAILY_POST_COUNT    = 'autoblogai_daily_post_count';

    public const ERROR_FAILED_PROMPT_ASSEMBLY  = 'autoblogai_prompt_assembly_failed';
    public const ERROR_INVALID_RESPONSE        = 'autoblogai_invalid_response';
    public const ERROR_BANNED_WORDS_DETECTED   = 'autoblogai_banned_words_detected';
    public const ERROR_DUPLICATE_CONTENT       = 'autoblogai_duplicate_content';
    public const ERROR_DAILY_CAP_EXCEEDED      = 'autoblogai_daily_cap_exceeded';
    public const ERROR_POST_CREATION_FAILED    = 'autoblogai_post_creation_failed';

    private $api_client;
    private $image_generator;
    private $logger;

    public function __construct( $api_client, $image_generator = null, $logger = null ) {
    ...
    public function generate_and_publish( $topic, $keyword = '', $args = array() ) {
        ...
        $publish_mode = $args['publish_mode'] ?? get_option( 'autoblogai_post_status', 'draft' );
        ...
        do_action( 'autoblogai_after_post_created', $post_id, $parsed, $topic, $keyword );
    ...
    private function assemble_prompt( $topic, $keyword, $locale, $tone ) {
        $prompt = apply_filters( 'autoblogai_prompt_before_assembly', '', $topic, $keyword, $locale, $tone );
...
PROMPT;
        ...
        $prompt = apply_filters( 'autoblogai_prompt_template', $prompt, $topic, $keyword, $locale, $tone );
    ...
    private function check_duplicate_content( $title, $content ) {
        ...
        $days_back = apply_filters( 'autoblogai_similarity_check_days', 30 );
    ...
    private function update_seo_metadata( $post_id, $content_data ) {
        ...
        update_post_meta( $post_id, '_autoblogai_meta_description', $content_data['meta_description'] );
        update_post_meta( $post_id, '_autoblogai_keywords', $content_data['keywords'] );
        ...
        update_post_meta( $post_id, '_autoblogai_generated', 1 );
        update_post_meta( $post_id, '_autoblogai_generated_at', current_time( 'mysql' ) );
    }
